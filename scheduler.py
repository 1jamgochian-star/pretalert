...
def verifica_alerte(produs_id, pret_curent, nume_produs, link_produs):
    try:
        conn = get_db()
        c = conn.cursor()
        c.execute("SELECT id, email, pret_dorit FROM alerte WHERE produs_id = %s AND activa = 1 AND pret_dorit >= %s", (produs_id, pret_curent))
        alerte = c.fetchall()
        conn.close()
        ...
        logging.error(f"❌ Eroare alerte: {e}")
...
def start_scheduler():
    ...
    scheduler.add_job(job_actualizare, 'interval', hours=12, id='actualizare_preturi')
    scheduler.start()
